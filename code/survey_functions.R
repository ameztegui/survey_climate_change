...
# ANOVA: comparison when dep. variables is continuous -------------------------------
  ...
  survey_comparison <- function (dataframe, dep, indep,  print_stats =F) {    
       # Subset data
...
# CHI-SQUARE: comparison when the dep. variable is categorical ----------------------
  ...
  chisq <- function (dataframe, dep, indep, leyenda, print_stats =F) {
       library(scales)            
...
#                
       ...
       if(print_stats == TRUE){
           print(summary(stats)) 
           print(chisq.post.hoc(stats,control='bonferroni', workspace=1e08))
       } else {
      }
       # Plot results
...
# Compare likert values across groups -------------------------------------
    ...
    for (i in seq_along(depen)) {
        ...
        if (grouping == "Stakeholder") {
            krus$groups$Stakeholder <- factor(row.names(krus$groups),
                                              levels=c("Federal Gov.", "Provincial Gov.",
                                                       "Industry", "Other private",
                                                       "Academia", "Student"))
        }
        
        if (grouping == "Province") {
            krus$groups$Province <- factor(row.names(krus$groups),
                                           levels=c("British Columbia", "Alberta", "Ontario",
                                                    "Quebec", "New Brunswick"))
        }
...
# mf_labeller -------------------------------------------------------------
         ...
         mf_labeller_bump <- function( value){
             value <- as.character(value)
             #adaptive practices
             value[value=="Natural_Regeneration" ] <-             "Allow forests to regenerate naturally  \nfollowing disturbance"
             value[value=="Old_growth_forests" ] <-               "Maintain large areas of old growth forests"
             value[value=="Reduce_differences_natural" ] <-       "Reduce the differences between managed  \nand natural forests"
             value[value=="Species_diversification" ] <-          "Promote species diversification (either at  \nthe stand or landscape level)"
             value[value=="Translocate_Populations" ] <-          "Active translocation of populations of a  \nspecies within that species' current range"
             value[value=="Assisted_migration" ] <-               "Assisted migration: Active translocation of  \nspecies outside its current range"
             value[value=="Seed_transfer" ] <-                    "Modify seed transfer zones/seed provenances"
             value[value=="Continuous_cover" ] <-                 "Modify management to maintain continuous  \ntree cover"
             value[value=="Provenance_tests" ] <-                 "Implement long-term provenance tests  \nalong latitudinal gradients"
             value[value=="Diverse_experiments" ] <-              "Diversify risk by implementing a diversity  \nof small-scale experiments"
             value[value=="Shorten_rotation" ] <-                 "Shorten silvicultural cycles (rotation  \nlength or more frequent partial cutting cycle)"
             value[value=="Thinning" ] <-                         "Use thinning to reduce stand density  \nand favor surviving individuals"
             value[value=="Salvage_logging" ] <-                  "Increase the amount of salvage logging  \nafter disturbances"
             value[value=="Promote_Retention" ] <-                "Increase the retention of structural  \nelements(green trees, snags)"
             value[value=="Timber_supply" ] <-                    "Include climate change in timber supply  \nanalysis"
             value[value=="Harvest_vulnerable_stands" ] <-        "Prioritize the harvesting of stands  \nmost vulnerable to natural disturbances"
             #Barriers to implementation
             value[value=="Uncertainties" ] <-                    "There are too many uncertainties on climate \n change impacts"
             value[value=="Practitioners_impact_low" ] <-         "Most practitioners think that the likely \n impact of CC will be low"
             value[value=="Lack_scientific_support" ] <-          "Lack of scientific support and knowledge \n transfer to forest practitioners"
             value[value=="Lack_info_local_scale" ] <-            "There is lack of information about CC impacts\n at local and regional scales"
             value[value=="Lack_solutions" ] <-                   "Lack of apparent tangible adaptive solutions\n (ground level practices)"
             value[value=="Lack_tools" ] <-                       "Lack of adequate tools for the inclusion of \n  climate change in forest planning"
             value[value=="Lack_workforce" ] <-                   "Lack of skilled and specialized workforce \nto use the tools needed"
             value[value=="Costs" ] <-                            "The cost of implementing adaptive measures\n  weakens the economic situation of enterprises"
             value[value=="Forest_companies_reluctant" ] <-       "Forest companies are reluctant to change\n their current management"
             value[value=="Lack_funding" ] <-                     "Lack of funding for the development of \n adaptation strategies"
             value[value=="Forest_certification" ] <-             "Excessive rigidity of most standards of \n forest certification"
             value[value=="Rigid_legislation" ] <-                "Excessively rigid legislation and \n public policies"
             value[value=="Lack_common_view" ] <-                 "Lack of a common, coherent view for all \n the forest sector"
             value[value=="Lack_of_policies" ] <-                 "Lack of policies that support innovation/\n diversification of practices"
             value[value=="Other_imminent_issues" ] <-            "An overwhelming collection of other imminent \n forestry issues compete with CC challenges"
